...
const addContact = catchAsyncError(async (req, res) => {
    const Id = req.user;
    const { organization_id, contact_email, phone_number, country_code } = req.body;
    ...
    const contactEmailExists = await contact_services.get_single_contact({ organization_id, contact_email, is_deleted: false });
    if (contactEmailExists) return response400(res, msg.emailIsExists);

    if (phone_number && country_code) {
        const phoneNumberExists = await contact_services.get_single_contact({ organization_id, phone_number, country_code, is_deleted: false });
...
const getContactList = catchAsyncError(async (req, res) => {
    ...
    const contactData = await contact_services.get_all_contacts(matchQuery, { __v: 0, updatedAt: 0 }, sortQuery);
...
const updateContact = catchAsyncError(async (req, res) => {
    const { organization_id, contact_id, contact_email, phone_number, country_code } = req.body;

    const organizationData = await organization_services.get_organization({ _id: organization_id, is_deleted: false, },);
    if (!organizationData) return response400(res, msg.organizationNotExists);

    const contactData = await contact_services.get_single_contact({ _id: contact_id, is_deleted: false, },);
    if (!contactData) return response400(res, msg.contactNotFound);

    if (contact_email) {
        const contactEmailExists = await contact_services.get_single_contact({ _id: { $ne: contact_id }, organization_id, contact_email, is_deleted: false });
        if (contactEmailExists) return response400(res, msg.emailIsExists);
    }

    if (phone_number && country_code) {
        const phoneNumberExists = await contact_services.get_single_contact({ _id: { $ne: contact_id }, organization_id, phone_number, country_code, is_deleted: false });
        if (phoneNumberExists) return response400(res, msg.phoneExists);
    }
...
const deleteContact = catchAsyncError(async (req, res) => {
    ...
    await contact_services.update_contact({ _id: contact_id }, { is_deleted: true });
