...
const addOrganization = catchAsyncError(async (req, res) => {
    ...
    const organizationExists = await organization_services.get_organization({ organization_name, is_deleted: false, added_by: Id });
...
const updateOrganization = catchAsyncError(async (req, res) => {
    ...
    const data = await organization_services.update_organization({ _id: organization_id }, req.body);
...
const deleteOrganization = catchAsyncError(async (req, res) => {
    ...
    await organization_services.update_organization({ _id: organization_id }, { is_deleted: true });

    await organization_services.update_many_member({ organization_id }, { is_deleted: true });
...
const addMember = catchAsyncError(async (req, res) => {
    ...
    const memberEmailExists = await organization_services.get_single_member({ organization_id, member_email, is_deleted: false });
    if (memberEmailExists) return response400(res, msg.emailIsExists);

    const memberPhoneExists = await organization_services.get_single_member({ organization_id, member_phone, is_deleted: false });
...
const updateMember = catchAsyncError(async (req, res) => {
    ...
    const memberData = await organization_services.get_single_member({ _id: member_id, is_deleted: false });
    if (!memberData) return response400(res, msg.memberNotExists);

    if (member_email) {
        const memberEmailExists = await organization_services.get_single_member({ _id: { $ne: member_id }, organization_id, member_email, is_deleted: false });
        if (memberEmailExists) return response400(res, msg.emailIsExists);
    }

    if (member_phone) {
        const memberPhoneExists = await organization_services.get_single_member({ _id: { $ne: member_id }, organization_id, member_phone, is_deleted: false });
        if (memberPhoneExists) return response400(res, msg.phoneExists);
    }

    const data = await organization_services.update_member({ _id: member_id }, req.body);
...
const deleteMember = catchAsyncError(async (req, res) => {
    ...
    await organization_services.update_member({ _id: member_id }, { is_deleted: true });
