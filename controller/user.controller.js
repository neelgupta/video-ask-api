...
const updateOne = catchAsyncError(async (req, res) => {
    ...
    let isEmailExits = await user_services.findUser({ email, _id: { $ne: userId } })
...
const fetchAll = catchAsyncError(async (req, res) => {
    let user = await user_services.getAllUsers({ select: "email name isActive createdAt", lean: true, sort: { createdAt: -1 } })
