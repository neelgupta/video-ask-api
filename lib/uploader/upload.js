...
const uploadVideoToCloudinary = async ({
  ...
  return new Promise((resolve, reject) => {
    const fileName = `${type}_${dayjs().unix()}`;
        ...
        eager: [
          {
            resource_type: "image",
            format: "jpg",
            transformation: [{ width: 200, height: 113, crop: "fill" }],
          },
        ],
      },
      async (error, result) => {
        if (error) {
          ...
          const fileSizeReadable = fileSizeBytes;

          if (type === "node" || type === "library") {
            await organization_services.update_organization(
              { _id: organization_id },
              { $inc: { storage_occupied: fileSizeReadable } }
            );
          }
          resolve({ videoUrl, thumbnailUrl, fileSize: fileSizeReadable });
        }
      }
    );
